...
public class EscFromTheMine extends ApplicationAdapter {
	...
	private void update() {
		...
		if (Gdx.input.isKeyPressed(Keys.A)) {objPlayer.x -= 100 * Gdx.graphics.getDeltaTime();}
		if (Gdx.input.isKeyPressed(Keys.D)) {objPlayer.x += 100 * Gdx.graphics.getDeltaTime();}
		if (Gdx.input.isKeyPressed(Keys.W)) {objPlayer.y += 100 * Gdx.graphics.getDeltaTime();}
		if (Gdx.input.isKeyPressed(Keys.S)) {objPlayer.y -= 100 * Gdx.graphics.getDeltaTime();}
